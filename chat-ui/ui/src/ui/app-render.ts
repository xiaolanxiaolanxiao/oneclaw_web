...
async function handleOpenWebUI(state: AppViewState) {
  ...
  if (window.oneclaw?.openExternal) {
    window.oneclaw.openExternal(fullUrl);
  } else {
...
function ensureSettingsEmbedBridge(state: AppViewState) {
  ...
  // ── Web 模式 Settings RPC 桥接 ──
  // 监听 settings iframe 的 RPC 请求，通过 Gateway WebSocket 转发
  window.addEventListener("message", async (event: MessageEvent) => {
    ...
    const data = event.data as
      | { source?: string; type?: string; id?: string; method?: string; params?: unknown }
      | undefined;
    if (!data || data.source !== "oneclaw-settings-rpc" || data.type !== "rpc-request") {
      return;
    }
    ...
    if (!replyTarget || !("postMessage" in replyTarget)) {
      return;
    }
    const client = (bridge.state as unknown as { client?: { request: (m: string, p?: unknown) => Promise<unknown> } }).client;
    if (!client) {
      replyTarget.postMessage({
        source: "oneclaw-chat-ui-rpc",
        type: "rpc-response",
        id,
        result: { success: false, data: null, message: "Gateway not connected" },
      }, "*");
      return;
    }
    try {
      ...
      replyTarget.postMessage({
        ...
        type: "rpc-response",
        id,
        result: { success: false, data: null, message: msg },
      }, "*");
    }
  });
