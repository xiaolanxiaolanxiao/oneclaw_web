...
(function () {
  ...
  // ── Web 模式兼容层 ──
  // 当 window.oneclaw 不存在时（非 Electron），通过 HTTP API 与 Vite dev server
  // 中间件通信，实现配置读写、API Key 验证等操作。
  if (!window.oneclaw) {
    // 调用服务端 API
    function settingsApi(route, params) {
      var url = "/__api/settings/" + route;
      var opts = params
        ? { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(params) }
        : { method: "GET" };
      return fetch(url, opts)
        .then(function (res) { return res.json(); })
        .catch(function (err) { return { success: false, data: null, message: err.message || "Network error" }; });
    }

    var webModeHandlers = {
      // ── 读取类 → GET /__api/settings/* ──
      settingsGetConfig: function () { return settingsApi("get-config"); },
      settingsGetChannelConfig: function () { return settingsApi("get-channel-config"); },
      settingsGetAdvanced: function () { return settingsApi("get-advanced"); },
      settingsGetAboutInfo: function () { return settingsApi("get-about-info"); },
      settingsGetWecomConfig: function () {
        return Promise.resolve({ success: true, data: { enabled: false, bundled: false, bundleMessage: "" } });
      },
      settingsGetDingtalkConfig: function () {
        return Promise.resolve({ success: true, data: { enabled: false, bundled: false, bundleMessage: "" } });
      },
      settingsGetQqbotConfig: function () {
        return Promise.resolve({ success: true, data: { enabled: false, bundled: false, bundleMessage: "" } });
      },
      settingsGetKimiConfig: function () {
        return Promise.resolve({ success: true, data: { enabled: false, botToken: "" } });
      },
      settingsGetKimiSearchConfig: function () {
        return Promise.resolve({ success: true, data: { enabled: false, apiKey: "", serviceBaseUrl: "" } });
      },
      settingsGetCliStatus: function () {
        return Promise.resolve({ success: true, data: { installed: false } });
      },
      settingsListConfigBackups: function () {
        return Promise.resolve({ success: true, data: { backups: [], hasLastKnownGood: false } });
      },
      settingsListFeishuPairing: function () {
        return Promise.resolve({ success: true, data: { requests: [] } });
      },
      settingsListFeishuApproved: function () {
        return Promise.resolve({ success: true, data: { entries: [] } });
      },
      settingsListWecomPairing: function () {
        return Promise.resolve({ success: true, data: { requests: [] } });
      },
      settingsListWecomApproved: function () {
        return Promise.resolve({ success: true, data: { entries: [] } });
      },

      // ── 验证类 → POST /__api/settings/verify-key ──
      settingsVerifyKey: function (params) { return settingsApi("verify-key", params); },

      // ── 保存类 → POST /__api/settings/* ──
      settingsSaveProvider: function (params) { return settingsApi("save-provider", params); },
      settingsSaveChannel: function (params) { return settingsApi("save-channel", params); },
      settingsSaveAdvanced: function (params) { return settingsApi("save-advanced", params); },
      settingsSaveWecomConfig: function (params) {
        return Promise.resolve({ success: false, message: "企业微信配置暂不支持 Web 模式" });
      },
      settingsSaveDingtalkConfig: function (params) {
        return Promise.resolve({ success: false, message: "钉钉配置暂不支持 Web 模式" });
      },
      settingsSaveQqbotConfig: function (params) {
        return Promise.resolve({ success: false, message: "QQ Bot 配置暂不支持 Web 模式" });
      },
      settingsSaveKimiConfig: function (params) {
        return Promise.resolve({ success: false, message: "KimiClaw 配置暂不支持 Web 模式" });
      },
      settingsSaveKimiSearchConfig: function (params) {
        return Promise.resolve({ success: false, message: "搜索配置暂不支持 Web 模式" });
      },

      // ── 其他 ──
      openExternal: function (url) { window.open(url, "_blank"); },
      getGatewayState: function () { return Promise.resolve("running"); },
      getUpdateState: function () { return Promise.resolve({ status: "idle" }); },
      checkForUpdates: function () { return Promise.resolve({ success: true }); },
    };

    window.oneclaw = new Proxy(webModeHandlers, {
      get: function (target, prop) {
        if (prop in target) {
          return target[prop];
        }
        return function () {
          console.warn("[Settings] Web mode: method not implemented:", prop);
          return Promise.resolve({ success: true, data: null });
        };
      }
    });

    console.info("[Settings] Running in web mode — using HTTP API bridge.");
  }
